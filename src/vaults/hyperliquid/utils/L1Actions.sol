...
contract L1Actions {
    ...
    function userVaultEquity(address user, address vault) public view returns (UserVaultEquity memory) {
        bool success;
        bytes memory result;
    ...
    function vaultTransfer(address vault, bool isDeposit, uint64 usd) public {
        IWrite(0x3333333333333333333333333333333333333333).sendVaultTransfer(vault, isDeposit, usd);
    }

    function vaultTransferWithBalanceRead(address vault, bool isDeposit, uint64 usd) external {
        UserVaultEquity memory equity = userVaultEquity(address(this), vault);
        uint64 oldEquity = equity.equity;
        console2.log("oldEquity", oldEquity);

        vaultTransfer(vault, isDeposit, usd);

        UserVaultEquity memory newEquity = userVaultEquity(address(this), vault);
        console2.log("newEquity", newEquity.equity);
    }
}
